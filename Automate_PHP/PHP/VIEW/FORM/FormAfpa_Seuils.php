...
echo '<div class="border center">';
	echo '<form class="GridFormSeuil" action="index.php?page=ActionAfpa_Seuils&mode='.$_GET['mode'].'" method="post"/>';
		...
		echo '<div class="caseForm donneeForm grid-columns-span-9">';
			echo '<select name="nature" '.$disabled.'>';
		...
		echo '<div class="caseForm donneeForm grid-columns-span-3"><input type="date" '.$disabled .'value="'.$elm->getDateSeuil().'" min="'.$dateDuJour->format('Y-m-d').'" name=DateSeuil pattern="'.$regex["*"].'"></div>';
		echo '<div></div>';
		echo '<label for=Temps class="caseForm labelForm">'.texte("Temps (mins)").'</label>';
		echo '<div class="caseForm donneeForm grid-columns-span-3"><input type="text" '.$disabled .'value="'.$elm->getTemps().'" name=Temps pattern="'.$regex["*"].'"></div>';
